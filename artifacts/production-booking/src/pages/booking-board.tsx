...
import { Settings2, CalendarDays, ChevronUp, ChevronDown, Plus, X, Pencil, Check, AlertTriangle, Ship, Clock, Trash2, RotateCcw } from "lucide-react";
import {
  Popover,
  PopoverContent,
  PopoverTrigger,
...
const TABS = ["300SL", "600SL", "1000/2000SL", "MetalsImpact"];
...
export default function BookingBoard() {
  ...
  // Summary metrics for the active tab.
  ...
  // Next available Sales Order booking = the earliest slot with no sales order
  // assigned whose ship date hasn't already passed.
  ...
  // Move a booking up (dir=-1) or down (dir=+1) the calendar. The swap runs
  // atomically server-side in one transaction, so there's no risk of a
  // partial/half-applied move. Dates and durations stay fixed with the slot.
  ...
  // Add a new slot at the end of the active tab's cadence. Continue the
  // bi-weekly rhythm by seeding its production start two weeks after the latest
  // dated slot; if no slot has a date yet, leave it blank for the planner.
  const handleAddSlot = () => {
  ...
  // Remove a single slot. Confirmed via AlertDialog in the slot's action menu.
  const handleDelete = (id: number) => {
    deleteSlot.mutate({ id }, { onSuccess: invalidateTab });
  };

  // Wipe the active tab and regenerate a fresh bi-weekly cadence. Destructive —
  // confirmed via AlertDialog before firing.
  const handleReset = () => {
    ...
    <div className="min-h-screen bg-background">
      <header className="border-b bg-card">
        <div className="container mx-auto px-4 h-16 flex items-center justify-between">
          <div className="flex items-center gap-2">
            ...
            <h1 className="text-xl font-bold tracking-tight">Production Planner</h1>
          </div>
        </div>
      </header>
      ...
      <main className="container mx-auto px-4 py-8">
        <Tabs value={activeTab} onValueChange={setActiveTab} className="w-full">
          ...
          {/* Summary bar — at-a-glance KPIs for the active tab */}
          ...
          {/* Cadence controls — add a slot or regenerate the whole cadence for the active tab */}
...
// Days between consecutive cadence slots (bi-weekly).
const CADENCE_INTERVAL_DAYS = 14;

// Add whole days to a YYYY-MM-DD date, returning YYYY-MM-DD (UTC-safe so it
// never drifts across DST/timezone boundaries).
...
// True when the date is strictly before today (local midnight). Today is never
// counted as past, matching the per-milestone circle coloring.
...
// Whole-week lead time from today to a ship date. Rounds away from zero so an
// overdue ship date keeps a negative sign instead of collapsing to 0.
...
// A slot is late when any of its milestone dates has passed and that milestone
// isn't checked off — the same rule that turns a progress circle red.
...
function SlotRow({ slot, activeTab, index, canMoveUp, canMoveDown, moving, deleting, onMoveUp, onMoveDown, onDelete }: { slot: any, activeTab: string, index: number, canMoveUp: boolean, canMoveDown: boolean, moving: boolean, deleting: boolean, onMoveUp: () => void, onMoveDown: () => void, onDelete: () => void }) {
  ...
  const handleDurations = (field: 'assyDays'|'packDays'|'pickDays', val: string) => {
    // Coerce blank/non-numeric to 0 and clamp negatives so the computed
    // Pick/Assy/Pack/Ship dates can never drift on bad input.
  ...
  const renderPhase = (
    ...
    // A milestone is late only when its OWN date has passed and it isn't checked
    // off. The bar/text turn red when either circle is late.
  ...
  // Allocate a Production Order. Item/status/delivery come from the order. If no
  // sales order is set on the slot yet, auto-link the order's sales order (and
  // its customer) — but never overwrite a sales order the planner already chose.
  ...
  // Clear only the Production Order. Keep any sales order/customer by re-sending
  // their current values so the server's clear-cascade doesn't null them.
  ...
  // Allocate / override the Sales Order independently of the production order.
  ...
  // Clear only the Sales Order. prodOrder is absent from the body, so the
  // server's clear-cascade does not trigger and production fields are kept.
  const handleClearSO = () => {
    updateSlot.mutate(
  ...
  return (
    <>
      ...
      >
        {/* Left Column: Schedule progress — Pick / Assy / Pack phases with check-off milestones */}
        ...
        {/* Middle Column: Allocation — Production Order & Sales Order side-by-side */}
