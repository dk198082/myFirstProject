...
import { useGetJobsByRegion } from "@workspace/api-client-react";
...
function JobRow({ job }: { job: RegionJob }) {
  return (
    <div
      className="py-3 border-b border-border last:border-0 grid grid-cols-1 sm:grid-cols-[1fr_auto] gap-2 group"
...
export default function JobsByRegion() {
  ...
  const { data: regions, isLoading, error } = useGetJobsByRegion(
    apiStatus ? { status: apiStatus } : {},
    { query: { queryKey: ["getJobsByRegion", apiStatus ?? "all"] } }
  );
