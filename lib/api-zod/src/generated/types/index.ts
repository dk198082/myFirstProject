/**
 * Generated by orval v8.9.1 🍺
 * Do not edit manually.
 * Api
 * Technician Dashboard API
 * OpenAPI spec version: 0.1.0
 */

export * from './bestFitTech';
export * from './booking';
export * from './contact';
export * from './countByLabel';
export * from './countByRegion';
export * from './createPlaceholderJob';
export * from './createScheduleBlock';
export * from './createScheduleBlockBlockType';
export * from './customer';
export * from './dashboardSummary';
export * from './deleteWbQueuedWritebacks200';
export * from './equipment';
export * from './errorResponse';
export * from './getJobsByRegionParams';
export * from './getResourceUtilizationParams';
export * from './getResourceUtilizationView';
export * from './getScheduleBoardGroupBy';
export * from './getScheduleBoardParams';
export * from './getScheduleBoardView';
export * from './getTechnicianByEmailParams';
export * from './getWbJobsByRegionParams';
export * from './getWbReportApprovedNotInvoicedParams';
export * from './getWbReportCompletedNotApprovedParams';
export * from './getWbReportWeeklyApprovedParams';
export * from './getWbResourceUtilizationParams';
export * from './getWbResourceUtilizationView';
export * from './getWbScheduleBoardGroupBy';
export * from './getWbScheduleBoardParams';
export * from './getWbScheduleBoardView';
export * from './healthStatus';
export * from './job';
export * from './listWbPlaceholderJobsParams';
export * from './listWbScheduleBlocksParams';
export * from './listWbServiceLocationsParams';
export * from './listWbWorkOrdersParams';
export * from './placeholderJob';
export * from './placeholderJobStatus';
export * from './regionGroup';
export * from './regionJob';
export * from './regionJobGroup';
export * from './reportFilters';
export * from './resourceUtilizationResponse';
export * from './scheduleBlock';
export * from './scheduleBlockBlockType';
export * from './scheduleBoard';
export * from './scheduleBoardView';
export * from './scheduledJob';
export * from './scheduleJob';
export * from './scheduleRegion';
export * from './scheduleTechnician';
export * from './searchWbJobsParams';
export * from './serviceOrderReport';
export * from './serviceOrderReportRow';
export * from './technician';
export * from './technicianJobGroup';
export * from './technicianJobsResponse';
export * from './technicianSummary';
export * from './technicianWithJobs';
export * from './technicianWorkOrderCount';
export * from './unscheduledJob';
export * from './unscheduledJobsResponse';
export * from './updatePlaceholderJob';
export * from './updateScheduleBlock';
export * from './updateScheduleBlockBlockType';
export * from './utilizationRegion';
export * from './utilizationTech';
export * from './wbBookingUpdate';
export * from './wbSaveResult';
export * from './wbSearchResult';
export * from './wbSearchResultType';
export * from './wbServiceLocation';
export * from './wbServiceLocationDetail';
export * from './wbSyncRequest';
export * from './wbSyncResult';
export * from './wbSyncResultResultsItem';
export * from './wbTechnician';
export * from './wbWorkOrder';
export * from './wbWriteback';
export * from './weeklyApprovedReport';
export * from './weeklyApprovedRow';
export * from './weeklyApprovedRowWeeks';
export * from './workOrderDetail';
export * from './workOrderProduct';
export * from './workOrderService';
