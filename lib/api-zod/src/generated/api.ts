...
export const CreateWbPlaceholderJobBody = zod.object({
  ...
  "color_index": zod.number().nullish().describe('0–18 palette index; null = match region default colour'),
  "start_time": zod.string().describe('ISO 8601 timestamp'),
  "end_time": zod.string().describe('ISO 8601 timestamp'),
  "notes": zod.string().nullish(),
  "status": zod.union([zod.enum(['Reminder Letter Sent', 'Quoted – No Purchase Order', 'Have Purchase Order', 'Have Credit Card', 'Cash in Advance', 'Credit Hold']),zod.null()]).optional()
...
export const UpdateWbPlaceholderJobBody = zod.object({
  ...
  "color_index": zod.number().nullish().describe('0–18 palette index; null to reset to region default colour'),
  "start_time": zod.string().optional().describe('ISO 8601 timestamp'),
  "end_time": zod.string().optional().describe('ISO 8601 timestamp'),
  "notes": zod.string().nullish(),
  "status": zod.union([zod.enum(['Reminder Letter Sent', 'Quoted – No Purchase Order', 'Have Purchase Order', 'Have Credit Card', 'Cash in Advance', 'Credit Hold']),zod.null()]).optional()
...
export const GetScheduleBoardResponse = zod.object({
  ...
  "postal_code": zod.string().nullish(),
  "country": zod.string().nullish(),
  "service_location_id": zod.string().nullish().describe('CRM service-location UUID for matching scheduled jobs to Potential Jobs.'),
 ...
 */
export const GetWbJobsByRegionQueryParams = zod.object({
  "status": zod.coerce.string().optional().describe('Filter by work order system status (e.g. Scheduled, Completed)')
})

export const GetWbJobsByRegionResponseItem = zod.object({
  "regionid_id": zod.string(),
  "region": zod.string(),
  "owner_name": zod.string().nullish(),
  "owner_email": zod.string().nullish(),
...
export const GetWbJobsByRegionResponse = zod.array(GetWbJobsByRegionResponseItem)


/**
 * @summary List d365crm work orders with status "Unscheduled" enriched with due date, duration, contact, and best-fit techs
 */
export const GetWbUnscheduledJobsResponse = zod.object({
  "jobs": zod.array(zod.object({
  "work_order_id": zod.string().nullish(),
  "work_order_number": zod.string().nullish(),
  "work_order_type": zod.string().nullish().describe('e.g. \"Calibration\/Service\" or \"Install\"'),
 ...
 */
export const GetWbResourceUtilizationQueryParams = zod.object({
  "start": zod.coerce.string().optional().describe('ISO date (YYYY-MM-DD) within the desired period'),
  "view": zod.enum(['week', 'month', 'quarter']).optional().describe('Period granularity — week (default), month, or quarter')
})

export const GetWbResourceUtilizationResponse = zod.object({
  "view": zod.string().describe('week | month | quarter'),
  "range_start": zod.string(),
  "range_end": zod.string(),
  "period_weeks": zod.number().describe('Number of weeks in the period (used to scale capacity display)'),
